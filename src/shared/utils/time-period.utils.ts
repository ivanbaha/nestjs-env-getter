 ...
 * - A finite, non-negative number (interpreted as milliseconds).
 ...
 * - If the input is a number, it is interpreted as milliseconds and converted to `resultIn`.
 ...
 * - Results are rounded **up** to the nearest integer (`Math.ceil`): `parseTimePeriod("1500", "s") === 2`.
 * @param value - The time period to parse, either a number (milliseconds) or a string with an optional time unit.
...
export function parseTimePeriod(value: string | number, resultIn: TimeMarker = "ms"): number {
  if (typeof value === "number") {
    return resultIn === "ms" ? value : Math.ceil(value / timeMarkersMapper[resultIn]);
  }
